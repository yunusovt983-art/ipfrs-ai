...
/// Async callback that executes one distributed-inference stage locally
/// (RoadMap Phase 5.1). The application (`ipfrs::Node`) installs this via
/// [`NetworkNode::set_activation_provider`] so inbound activation requests can be
/// computed against the local numeric engine. Typically wraps
/// `ipfrs_tensorlogic::distributed::execute_stage`.
pub type ActivationProvider = Arc<
    dyn Fn(
            crate::activation::StageRequest,
        ) -> std::pin::Pin<
            Box<dyn std::future::Future<Output = crate::activation::StageResponse> + Send>,
        > + Send
        + Sync,
>;

/// In-flight outbound activation stages: request id → reply channel.
type PendingActivation = Arc<
    Mutex<HashMap<OutboundRequestId, oneshot::Sender<IpfrsResult<crate::activation::StageResponse>>>>,
>;

/// A cheap, `Send + Sync` handle for running distributed-inference stages on peers
/// (RoadMap Phase 5.1).
///
/// `NetworkNode` owns the libp2p `Swarm` and is therefore `!Sync`, so it cannot be
/// borrowed across the `await` points of the async `ActivationTransport` seam. This
/// handle holds only the pieces the transport actually needs — a clone of the swarm
/// command channel and the connected-peer set, both `Send + Sync` — and implements
/// [`ActivationTransport`](ipfrs_tensorlogic::distributed::transport::ActivationTransport)
/// so it can be handed straight to `execute_pipeline`. Obtain one with
/// [`NetworkNode::activation_handle`].
#[derive(Clone)]
...
impl ipfrs_tensorlogic::distributed::transport::ActivationTransport for ActivationHandle {
    async fn run_stage(
        &self,
        peer: &str,
        req: crate::activation::StageRequest,
    ) -> std::result::Result<
        crate::activation::StageResponse,
        ipfrs_tensorlogic::computation_graph::GraphError,
    > {
        use ipfrs_tensorlogic::computation_graph::GraphError;

        let peer_id: PeerId = peer
            .parse()
            .map_err(|e| GraphError::ExecutionError(format!("invalid peer id '{peer}': {e}")))?;

        // A stage pinned to this very node runs in-process — libp2p cannot dial
        // self, and the request is self-describing, so this is the same result a
        // remote peer would compute, without a network round-trip.
        if peer_id == self.local_peer {
            return Ok(ipfrs_tensorlogic::distributed::wire::execute_stage(&req));
        }
        ...
        self.cmd_tx
            .try_send(SwarmCommand::RunStage {
                peer: peer_id,
                request: Box::new(req),
                reply: tx,
            })
            .map_err(|e| {
                GraphError::ExecutionError(format!("swarm command channel error: {e}"))
            })?;

        match tokio::time::timeout(Duration::from_secs(120), rx).await {
            Ok(Ok(Ok(resp))) => Ok(resp),
            Ok(Ok(Err(e))) => {
                Err(GraphError::ExecutionError(format!("activation to {peer}: {e}")))
            }
            Ok(Err(_)) => Err(GraphError::ExecutionError(
                "activation reply channel closed".to_string(),
            )),
            Err(_) => Err(GraphError::ExecutionError(format!(
                "activation to {peer} timed out"
            ))),
        }
    }
}
...
pub struct IpfrsBehaviour {
    ...
    >,
    /// Distributed activation streaming request-response (RoadMap Phase 5.1)
    pub activation: request_response::cbor::Behaviour<
        crate::activation::StageRequest,
        crate::activation::StageResponse,
    >,
}
...
pub enum IpfrsBehaviourEvent {
    ...
    Activation(
        request_response::Event<crate::activation::StageRequest, crate::activation::StageResponse>,
    ),
}
...
impl
    From<request_response::Event<crate::activation::StageRequest, crate::activation::StageResponse>>
    for IpfrsBehaviourEvent
{
    fn from(
        event: request_response::Event<
        ...
        IpfrsBehaviourEvent::Activation(event)
    }
}
...
enum SwarmCommand {
    ...
    /// Run one distributed-inference stage on a peer (RoadMap Phase 5.1).
    /// The `StageRequest` is boxed — it carries a whole subgraph + tensors and is
    /// far larger than the other variants.
    RunStage {
        peer: PeerId,
        request: Box<crate::activation::StageRequest>,
        reply: oneshot::Sender<IpfrsResult<crate::activation::StageResponse>>,
    },
}
...
pub struct NetworkNode {
    ...
    /// Application callback to execute inbound activation stages on the local
    /// numeric engine (RoadMap Phase 5.1). `None` → inbound stages return an error.
    activation_provider: Arc<parking_lot::RwLock<Option<ActivationProvider>>>,
...
impl NetworkNode {
    ...
    pub fn new(config: NetworkConfig) -> IpfrsResult<Self> {
        ...
        Ok(Self {
            ...
            activation_provider: Arc::new(RwLock::new(None)),
    ...
    fn build_swarm(
        ...
        // Create distributed activation-streaming request-response (RoadMap Phase 5.1).
        // Longer timeout than the others: a stage includes remote numeric execution.
        let activation = request_response::cbor::Behaviour::new(
            [(
                StreamProtocol::new(crate::activation::PROTOCOL),
                ProtocolSupport::Full,
            )],
            request_response::Config::default()
    ...
    pub async fn start(&mut self) -> IpfrsResult<()> {
        ...
        let activation_provider = Arc::clone(&self.activation_provider);
        ...
        // In-flight outbound activation stages (RoadMap Phase 5.1).
        let pending_activation: PendingActivation = Arc::new(Mutex::new(HashMap::new()));
            ...
            loop {
                tokio::select! {
                    event = swarm.select_next_some() => {
                        Self::handle_swarm_event(event, &event_tx, swarm.behaviour_mut(), &external_addrs, &connected_peers, &provider_waiters, &nat_metrics, &block_provider, &pending_fetch, &peer_rtt, &peer_region, &peer_load, &semsearch_provider, &pending_semsearch, &activation_provider, &pending_activation).await;
                    }
                    Some(cmd) = swarm_cmd_rx.recv() => {
                        Self::handle_swarm_command(cmd, &mut swarm, &provider_waiters, &pending_fetch, &pending_semsearch, &pending_activation).await;
                    }
                    _ = shutdown_rx.recv() => {
    ...
    async fn handle_swarm_event(
        ...
        activation_provider: &Arc<RwLock<Option<ActivationProvider>>>,
        ...
        match event {
            ...
            // Distributed activation streaming request-response (RoadMap Phase 5.1).
            SwarmEvent::Behaviour(IpfrsBehaviourEvent::Activation(
                request_response::Event::Message { message, .. },
            )) => match message {
                // Inbound: a peer asks us to run one stage on our numeric engine.
                request_response::Message::Request {
                    request, channel, ..
                } => {
                    let provider = activation_provider.read().clone();
                    let resp = match provider {
                        Some(f) => f(request).await,
                        None => crate::activation::StageResponse::err(
                            "no activation provider installed on this peer",
                        ),
                    };
                    let _ = behaviour.activation.send_response(channel, resp);
                }
                // Outbound: a response to our stage → wake the waiter.
                request_response::Message::Response {
                    request_id,
                    response,
                } => {
                    if let Some(reply) = pending_activation.lock().await.remove(&request_id) {
                        let _ = reply.send(Ok(response));
                    }
                }
            },
            // Activation outbound failure → fail the waiter if present.
            SwarmEvent::Behaviour(IpfrsBehaviourEvent::Activation(
                request_response::Event::OutboundFailure { request_id, error, .. },
            )) => {
                if let Some(reply) = pending_activation.lock().await.remove(&request_id) {
                    let _ = reply.send(Err(ipfrs_core::error::Error::Network(format!(
                        "activation stage request failed: {}",
                        error
                    ))));
                }
            }
    ...
    async fn handle_swarm_command(
        ...
        match cmd {
            ...
            SwarmCommand::RunStage {
                ...
                pending_activation.lock().await.insert(id, reply);
                debug!("Activation stage request sent to {}", peer);
            }
        }
    ...
    /// Install the callback that executes inbound activation stages on the local
    /// numeric engine (RoadMap Phase 5.1). Without it, inbound stages return a
    /// `StageResponse` error to the caller.
    pub fn set_activation_provider(&self, provider: ActivationProvider) {
        *self.activation_provider.write() = Some(provider);
    }

    /// Obtain a `Send + Sync` [`ActivationHandle`] for driving distributed
    /// pipelines across peers (RoadMap Phase 5.1). Returns an error if the node
    /// has not been started (no swarm command channel yet).
    pub fn activation_handle(&self) -> IpfrsResult<ActivationHandle> {
        let cmd_tx = self.swarm_cmd_tx.clone().ok_or_else(|| {
            ipfrs_core::error::Error::Network("network node not started".to_string())
        })?;
        Ok(ActivationHandle {
            cmd_tx,
    ...
    /// Run one distributed-inference stage on a connected peer over
    /// `/ipfrs/activation/1.0.0`. Returns the peer's [`StageResponse`] (which may
    /// itself carry a stage-level error), or a transport error on
    /// timeout/disconnect.
    ///
    /// [`StageResponse`]: crate::activation::StageResponse
    pub async fn run_stage_on_peer(
        &self,
        peer: &PeerId,
        request: crate::activation::StageRequest,
    ) -> IpfrsResult<crate::activation::StageResponse> {
        ...
        let (tx, rx) = oneshot::channel();
        self.send_swarm_cmd(SwarmCommand::RunStage {
            peer: *peer,
