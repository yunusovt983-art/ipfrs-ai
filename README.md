...
> → block-fetch у ближайшего пира (по RTT/региону) → backfill + verify по CID.
...
| Крейт | Файлы | Строки |
|-------|------:|------:|
| `ipfrs-tensorlogic` | 216 | 157,078 |
| `ipfrs-network` | 229 | 157,576 |
...
> **701,511 строк** Rust в **1,012 файлах** в 12 крейтах (без артефактов сборки).  
> **730 файлов** содержат `#[cfg(test)]` — обширное инлайн-покрытие тестами.  
> **~193 внешних зависимости** в 15 файлах `Cargo.toml`.  
> **Расположение**: `ipfrs_source/` (перенесено из `Vendor/ipfrs`).  
> _Пересчитано 2026-06-21 — выверено по рабочему дереву (после geo-инференса: network/tensorlogic/interface/node)._
...
| **Итого (архитектура)** | **61** | **23,776** |

> **23,776 строк** документации архитектуры в **61 Markdown-файле** (7 баз знаний + корневые доки + RoadMap с geo-инференсом).  
...
> Всего Markdown в репозитории (без `target/`, `Vendor/`): **135 файлов, 58,155 строк** — остальное это доки исходников и служебное.  
> _Пересчитано 2026-06-21 — выверено по рабочему дереву._
