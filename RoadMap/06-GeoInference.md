...
| **Фаза 3 (load-сигнал)** — `/ipfrs/load` gossip → `peer_load` → score; `advertise_load`/`subscribe_peer_load` | `ipfrs-network/src/node.rs`, `ipfrs/src/node/geo_ops.rs` | _текущий_ |
...
Осталось по фазам: 5 (распределённое исполнение графа — заблокировано отсутствием
численного tensor-executor'а + циклом network↔tensorlogic), 6 (FedAvg по регионам).
**Фаза 3 полностью**: RTT ✅ + регион ✅ + residency ✅ + load ✅. (Внешние API: GraphQL `geo_fetch` ✅ + gRPC `GeoService.GeoFetch` ✅.
