...
> **🔎 Уточнение по коду (2026-06-19).** Сам алгоритм HNSW **делегирован крейту `hnsw_rs`**
> (`hnsw.rs:6,126`); backend смонтирован на `DistL2`, а cosine/dot **эмулируются** нормализацией
> (`hnsw.rs:379`). Метрики Jaccard **нет**; дефолт `with_defaults` — L2 (не cosine). Повторная
> вставка существующего CID — **ошибка**, не upsert (`hnsw.rs:167`). ⚠️ Реальные баги:
> `VectorIndex::rebuild` молча опустошает граф (`hnsw.rs:586`); снапшот не сохраняет топологию
> HNSW (`hnsw.rs:791`); `DiskANNIndex::compact` — no-op; распределённый поиск через семантический
> DHT — заглушка (`dht_node.rs:409`). ✅ Зато логико-семантический fallback **реализован**
> (`solver.rs:340`). Детали: `[[12-RealityCheck]]`, `[[../Wiki/06-SemanticContext]]`.
