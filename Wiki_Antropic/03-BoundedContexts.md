...
> **🔎 Уточнение по коду (2026-06-19).** Точнее: **1 Shared Kernel (`ipfrs-core`) + 5
> доменных контекстов + 1 прикладной/шлюзовой** (`ipfrs` + `ipfrs-interface`). «Полная
> независимость» — идеал; на практике есть две перекрёстные связи: `network → tensorlogic`
> (распределённый вывод через gossip, `node.rs:32`) и `semantic → tensorlogic` (логико-
> семантический мост, `solver.rs:12`). Структурный паттерн всех крупных контекстов —
> **малое рабочее ядро + огромный спутниковый слой**, часто не подключённый к живому пути.
> Полная DDD-карта: `[[../Wiki/02-StrategicDesign]]`; реестр заглушек: `[[12-RealityCheck]]`.
