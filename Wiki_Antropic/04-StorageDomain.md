...
> **🔎 Уточнение по коду (2026-06-19).** Инвариант `hash(data)==cid` enforced при
> **создании** (`block.rs:70`), но ⚠️ **НЕ проверяется при чтении**: `get` использует
> `Block::from_parts(*cid, data)` без `verify()` (`blockstore.rs:350`) — тихая порча диска
> отдаётся как валидные данные. Реальный собранный стек декораторов — **`Bloom → Cache →
> Sled`** (`helpers.rs:136`), Bloom самый внешний (не `Cache→Tiering→Sled`). `SledBlockStore`
> имеет всего **3 поля** (`db, dedup_stats, compaction_scheduler`), без встроенного кэша.
> Существуют **три разных GC**, и возрастной порог `min_age` enforced только в одном из них.
> `CircuitBreaker` — standalone-утилита, **не** декоратор. Детали: `[[12-RealityCheck]]`,
> `[[../Wiki/04-StorageContext]]`.
